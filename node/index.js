...
const createTable = async () =>{
    await client.query(`CREATE TABLE IF NOT EXISTS users(id serial PRIMARY KEY, 
        name VARCHAR (255) UNIQUE NOT NULL, email VARCHAR (255) UNIQUE NOT NULL, 
        age INT NOT NULL );`)
}; //定義一個非同步函數
...
app.get('/api/from', async(req, res) => {
    try{
        ...
        const response = await client.query(`INSET INTO users(name, email, age) VALUES('${name}', '${email}', '${age}')`);
        if (response){
